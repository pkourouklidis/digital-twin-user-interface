...
import { updateDeployment } from '../../thunks';
import Grid from '@material-ui/core/Grid';
import KubernetesLogo from '../ProjectElements/KubernetesLogo';

const EditDeployment = ({ onClose, open, project, startUpdatingDeployment, editData }) => {
    ...
    useEffect(() => {
        setDeploymentName(editData.name);
        setDeploymentDescription(editData.description);
        setDeploymentUrl(editData.url);
        setDeploymentToken(editData.token);
        setDeploymentId(editData.nameSpaceId);
    }, [editData])
    ...
    const handleApply = (name, id, description, url, token) => {
        ...
        if (!invalid) {
            ...
            startUpdatingDeployment(deploymentDetails, project.id, editData.deploymentId, auth);
        }
    };
...
const mapDispatchToProps = dispatch => ({
    startUpdatingDeployment: (deploymentDetails, projectId, deploymentId, authContext) => dispatch(updateDeployment(deploymentDetails, projectId, deploymentId, authContext))
})
